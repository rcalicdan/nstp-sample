<div>
    ...
    <div x-data="{
        ...
        <x-ui.filter-bar>
            ...
            <select wire:model.live="gender" class="border border-[#f9e6ec] rounded px-3 py-2.5 text-sm bg-[#fdf2f5]/50 min-w-[130px] transition focus:border-[#800033] focus:outline-none">
            ...
            <select wire:model.live="schoolYear" class="border border-[#f9e6ec] rounded px-3 py-2.5 text-sm bg-[#fdf2f5]/50 min-w-[160px] transition focus:border-[#800033] focus:outline-none">
        ...
        <x-ui.table-card title="EVSU NSTP-ROTC Masterlist" :countText="'Showing ' . $this->students->count() . ' record(s) on this page'">
            <x-table.main>
                <x-table.thead>
                    <x-table.tr>
                ...
                <x-table.tbody>
                    @forelse($this->students as $index => $student)
                        <x-table.tr wire:key="rotc-student-{{ $student->id }}">
                            ...
                            <x-table.td class="hidden md:table-cell">
                                <span class="inline-flex items-center text-[0.68rem] font-bold tracking-wider px-2 py-0.5 rounded bg-[#2d0012]/10 text-[#4a001c] border border-[#f9e6ec] uppercase" title="{{ $student->course_label }}">
                                    {{ $student->course }}
                                </span>
                            </x-table.td>
                            <x-table.td class="hidden md:table-cell">
                                ...
                                @else
                                    <span class="inline-flex items-center text-[0.68rem] font-bold tracking-wider px-2 py-0.5 rounded bg-gray-100 text-gray-500 border border-gray-200 uppercase">{{ $student->gender->value }}</span>
                                @endif
                            </x-table.td>
                            <x-table.td class="text-gray-500 text-xs hidden lg:table-cell">{{ $student->schoolYear?->label ?? '—' }}</x-table.td>
                                ...
                                <div class="flex items-center justify-center gap-1.5">
                                    ...
                                    @can('delete', $student)
                                        <x-utils.delete-button 
                                            :message="'Are you sure you want to permanently remove ' . $student->first_name . ' ' . $student->last_name . '?'"
                    ...
                    @empty
                        <x-table.empty colspan="9" title="No ROTC Cadets Found" description="Adjust your search or filters." />
                    @endforelse
                </x-table.tbody>
            </x-table.main>
        ...
        @include('livewire.rotc-students.partials.view-modal')
        @include('livewire.rotc-students.partials.create-modal')
        @include('livewire.rotc-students.partials.edit-modal')
        @include('livewire.rotc-students.partials.csv-history-modal')
    </div>
</div>