...
@php
    $baseClasses =
        'inline-flex items-center justify-center font-display tracking-widest uppercase rounded-lg transition-all duration-150 disabled:opacity-70 disabled:cursor-not-allowed group active:scale-[.98]';
...
<button type="{{ $type }}" {{ $attributes->merge(['class' => "$baseClasses $sizeClasses $colorClasses"]) }}
    ...
    <span class="flex items-center gap-2" wire:loading.class="hidden"
        @if ($attributes->has('wire:target')) wire:target="{{ $attributes->get('wire:target') }}" @endif>
        {{ $slot }}
    </span>

    @if ($showSpinner)
        <span class="items-center gap-2 hidden" wire:loading.class.remove="hidden" wire:loading.class="flex"
            @if ($attributes->has('wire:target')) wire:target="{{ $attributes->get('wire:target') }}" @endif>
